...
class VendingMachine:
    ...
    def vend(self):
        if not self.stock:                      # if stock is empty
            return 'Inventory empty. Restocking required.'
        elif self.balance < self.price:     # if do not have enough money
            return 'You must add ${0} more funds.'.format(self.price - self.balance)        # use string formatting syntax
        elif self.stock and self.balance >= self.price:         # if has product and enough money
            self.stock -= 1                
            self.balance -= self.price      # calc the change
            if not self.balance:            # no change 
