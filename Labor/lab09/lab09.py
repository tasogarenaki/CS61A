...
def subseqs(s):
    ...
    if not s:                                           # if list is empty      
        ...
        rest = subseqs(s[1:])                           # the rest elements with recursion 
...
def inc_subseqs(s):
    ...
    def subseq_helper(s, prev):
        if not s:
            return [s]
        elif s[0] < prev:                           # if curr smaller than prev one
            return subseq_helper(s[1:], prev)       # compair the prev one with the rest of elements
        else:
            a = subseq_helper(s[1:], s[0])          # include s[0]
            b = subseq_helper(s[1:], prev)          # exclude s[0]
            return insert_into_all(s[0], a) + b     # insert curr element in a and add the prev lists
...
class Keyboard:
    ...
    def __init__(self, *args):
        #self.buttons = {}                               # create a dictionary
        #for button in args:
        #    self.buttons[button.pos] = button           # put the key with position in the dic
    ...
    def press(self, info):
        ...
        if info < len(self.buttons):
            self.buttons[info].times_pressed += 1         # how many times button pressed
            return self.buttons[info].key                 # return the key at position 
        return ''                                         # if out of the range


    def typing(self, typing_input):
        """Takes in a list of positions of buttons pressed, and
        returns the total output"""
        output = ''                                       # key inside of ' ' 
        for pos in typing_input:
            self.buttons[pos].times_pressed += 1          # count total times button pressed (inc. list here)
            output += self.buttons[pos].key               # build the total output
        return output 
...
def trade(first, second):
    ...
    while m <= len(first):
        n = 1
        while n <= len(second):
            if sum(first[:m]) is sum(second[:n]):                 
...
def shuffle(cards):
    ...
    half = len(cards) // 2                                      # length of half cards 
    shuffled = []
    for i in range(half):
        shuffled += [cards[:half][i], cards[half:][i]]          # the first half and second half
    return shuffled
