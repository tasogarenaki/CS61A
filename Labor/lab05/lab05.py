...
def distance(city_a, city_b):
    ...
    # below can not pass the doctest (must use get_xxx)
...
def riffle(deck):
    ...
    """
    "*** YOUR CODE HERE ***"
    # for-statment can not use 'range(deck)', cause the input 'deck' may as range!
    return [deck[i//2] if i % 2 == 0 else deck[i//2 + len(deck)//2] for i, num in enumerate(deck)]
...
def add_trees(t1, t2):
    ...
    # add the roots of both trees directly
    # for branches via recursion, and because of that the next iteration: 'branches' -> 'roots'
    ...
    # because of the different number of branches (len), 1) only add the lowst layer
    # the simplest way may add brauches with value 0 for the short tree, so now they both have same branches
