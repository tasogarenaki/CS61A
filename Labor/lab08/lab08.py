...
def convert_link(link):
    ...
    # iterative solution
    lst = []
    while link:                         # link != link.empty
        lst.append(link.first)
        link = link.rest                # let the next iteration go with .rest
    return lst
    
    # recursive solution
    def subfunc(link):
        if not link:                    # link == link.empty
            return []
        else:
            return [link.first] + subfunc(link.rest)
    return subfunc(link)                # invoke the subfunc
